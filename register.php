...
<section class="card">
<h1>Registrace</h1>
<p class="lead" style="color: var(--muted);">Vytvořte si hráčský účet.</p>
    ...
    <div class="input-field">
        <div class="select-wrap">
            <select name="gender" id="gender" required>
                ...
                <option value="O">Jiné / neuvádět</option>
            </select>
        </div>
    </div>
</label>
...
<div class="file-upload-wrap input-wrap">
<input type="file" name="photo" id="photo"
required accept="image/*">

<label for="photo" class="custom-file-btn" style="margin-right: 15px;">Vybrat soubor</label>
<span class="file-name-display" id="fileNameDisplay">Soubor nevybrán</span>
</div>
<small style="display: block; margin-top: 5px; margin-bottom: 25px; color: var(--muted);">Musí být obrázek, bude převeden na JPEG 800×XXX, kvalita 90.</small>
</label>
...
<script>
// Zajištění funkčnosti Custom File Input
document.addEventListener('DOMContentLoaded', () => {
const registerForm = document.getElementById('registerForm');
const photoInput = document.getElementById('photo');
const fileNameDisplay = document.getElementById('fileNameDisplay');

// Zobrazení názvu souboru pro Custom File Input
...
// Původní validace (upraveno pro použití nového ID 'gender')
registerForm.addEventListener('submit', function (e) {
const email = document.getElementById('email').value.trim();
const phone = document.getElementById('phone').value.trim();
const gender = document.getElementById('gender').value; // Nové
...
// ... (Ostatní validace zůstávají stejné) ...
      // Pokud se validace Pohlaví dělala dříve přes radio buttons,
      // nyní stačí, že je select nastaven jako 'required'.
...
errors.push('Telefon musí mít 9–20 číslic (případně s + a mezerami).');
      ...
      // Validace Pohlaví (pokud necháte první option prázdnou)
      if (gender === "") {
        errors.push('Vyberte pohlaví.');
      }
